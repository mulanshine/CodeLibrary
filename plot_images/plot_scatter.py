...
plt.scatter(result['CPU Time'],result['Dice'],c = cValue_2, s=100, marker='.', cmap=cm) # s 设置大小 marker='.' or '*'

# 添加数据标签
for a,b,c in zip(result['CPU Time'],result['Dice'],result['Model']):
    plt.text(a, b+0.05, c, ha='center', va= 'bottom',fontsize=8)
...
plt.colorbar().set_label(u'P (M)',fontdict=font)
plt.savefig("/root/chujiajia/Result_IMG/img.png")
