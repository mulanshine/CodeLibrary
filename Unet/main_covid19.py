...
def bz_dice_loss(batchsize, input, target):
    eps = 1e-7
    iflat = input.view(batchsize,-1)
    tflat = target.view(batchsize,-1)
    intersection = iflat * tflat #(bz, wh)
    intersection = intersection.sum(dim=-1) # (bz,1)
    union = iflat.sum(dim=-1) + tflat.sum(dim=-1) # (bz,1)
    dice = (2.0 * intersection) / (union + eps) # (bz, 1)
    dice = dice.sum() / (batchsize + eps)
    return 1 - dice
...
def contour_loss(input, target, edge):
    # eps = 0.0001
    eps = 1e-7
    input = input * edge.cuda()
    target = torch.Tensor(target * edge).cuda()
    iflat = input.view(-1)
    tflat = target.view(-1)
    intersection = (iflat * tflat).sum()
    union = iflat.sum() + tflat.sum()
...
def run_main():
    ...
    parser.add_argument('--gpu', type=int, default=1, help='gpu ids: e.g. 0  0,1,2, 0,2. use -1 for CPU')
    parser.add_argument('--initial_lr', type=float, default=0.001)
    parser.add_argument('--resultpath', type=str, default="/root/chujiajia/Results/")
    parser.add_argument('--experiment_name', type=str,default='bz12_bzdice_baseline_covid_epoch100')
    ...
    traindataset = Covid19DataSet(args.dataroot, set="train",lesion_phase=True,augmentations=None)
    trainloader = DataLoader(traindataset, batch_size=args.batchsize,shuffle=True, num_workers=args.num_workers, pin_memory=True)
    testdataset = Covid19DataSet(args.dataroot, set="test",lesion_phase=True,augmentations=None)
